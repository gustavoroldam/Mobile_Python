...
# Criar uma tela sempre acima do "GUI"
...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            with open("refreshtoken.txt", "r") as arquivo:
                refrest_token = arquivo.read()
            local_id, id_token = self.firebase.trocar_token(refrest_token)
            self.local_id = local_id
            self.id_token = id_token
            # Pegar informações do usuário
            requisicao = requests.get(f"https://aplicativovendashash-2cfc7-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requests.patch(f"https://aplicativovendashash-2cfc7-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)
        #Opcional (Voltar a tela depois de mudar)
        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-2cfc7-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                if id_vendedor_adicionado == self.id_vendedor:
                    mensagem_texto.text = "Você não pode se auto adicionar na equipe"
                else:
    ...
    def selecionar_cliente(self, foto, *args):
        ...
        for item in list(lista_clientes.children):
            ...
            # foto -> carrefour.png / Label -> Carrofour -> carrefour -> carrefour.png
            try:
                texto = item.text
    ...
    def adicionar_venda(self):
        ...
        # dado tudo certo isso vai adicionar vendas
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade) == float):
            ...
            info = f'{{"cliente":"{cliente}", "produto":"{produto}",' \
                   f' "foto_cliente":"{foto_cliente}",' \
                   f'"foto_produto":"{foto_produto}", "data":"{data}", ' \
                   f'"unidade":"{unidade}","preco":"{preco}", ' \
                   f'"quantidade":"{quantidade}"}}'

            requests.post(f"https://aplicativovendashash-2cfc7-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}", data=info)
            ...
            requisicao = requests.get(f"https://aplicativovendashash-2cfc7-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://aplicativovendashash-2cfc7-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
        ...
        for local_id_usuario in requisicao_dic:
            try:
                ...
                for id_venda in vendas:
                    ...
                    banner = BannerVenda(cliente=venda["cliente"], produto=venda["produto"],foto_cliente=venda["foto_cliente"], foto_produto=venda["foto_produto"],
...
# Os codigos padroes de todos os app
# from kivy.app import App
# from kivy.lang import Builder
# GUI = Builder.load_file("main.kv")
# class MainApp(App):
...
# post -> cria/envia informações
# patch -> atualiza informações
